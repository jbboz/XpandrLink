...
- Content-hash deduplication (FNV-1a) with amber `[D]` marker + "Rm Dupes" action.
...
both a Matrix-12 **and** an Xpander were captured transmitting `[id] = 0x02` — Matrix-12
`F0 10 02 0A 00 18 00 01 00 00 00 F7`, Xpander `F0 10 02 0A 00 18 00 7E 01 00 00 F7`. Both
models default to `0x02`, so the byte cannot tell them apart.

> An earlier draft of this document claimed `0x02` = Xpander / `0x04` = Matrix-12 as a
> firmware-fixed model code. That was inferred from the upstream C# app (which hardcodes
> `0x02`) and from a report where a Matrix-12 ignored `0x02`-stamped output — but the real
> cause there was a fresh-install `SysexID` of `0`, not a model mismatch. The direct
...
**JUCE version**: developed and tested against **JUCE 8.0.12**. The sibling-directory setup (below) doesn't pin a version by itself — CI (`.github/workflows/build.yml`) checks out JUCE at tag `8.0.12` explicitly for reproducibility; local dev checkouts should match this tag to avoid drift. If bumping the JUCE version, update both this line and the CI workflow's `ref:` together.

### macOS prerequisites
- Xcode (for the compiler and SDK)
- CMake 3.22+ (`brew install cmake`)
- JUCE 8 cloned as a sibling, at the pinned tag: `git clone --branch 8.0.12 https://github.com/juce-framework/JUCE ../JUCE`
