...
> Sessions **A** (Matrix-12 hardware), **B** (Xpander per-feature coverage), and **C** (morphing)
> all confirmed clean. Sessions D (DAW CC automation) and E (Windows VST3-in-DAW) remain open and
> may stagger post-launch, with the README marking Windows VST3 as untested-in-hosts until E closes.
...
| **Hardware display banner** | Low (display-only; can't corrupt patch memory) | Implemented (`MidiEngine::sendDisplayMessage`/`sendDisplayOff`/`sendDisplayOn`, DISPLAY field in `MidiSettingsPanel`) but never sent to real hardware. Needs one confirmation per model (Xpander cmd `0x05`, Matrix-12 cmd `0x06`) that the front-panel VFD actually displays the text correctly — wrong command byte or off-by-one in the 80-byte padding would show garbage or nothing. |
