/*
  ...
  Also covers a fix ported from the XpandrLink curated clone (commit 3797f5b, found
  there 2026-07-15, never ported back until now): getCurrentModBytes() was zero-filling
  unused mod-matrix slots instead of using the hardware's empty-slot sentinel
  (source=0x1F, dest=0x3F) -- the same corruption class as the Init Patch mod-matrix
  bug, but live in the editor's own Save Patch / Store-to-hardware-slot encoding path.
  Also adds FullModMatrixPanel::compactSlots(), called after removeEntry/removeAtSlot,
  so a removed entry's grid position doesn't leave later entries with a gap.

  Verifies: NFR-HW-01, NFR-HW-05 (see docs/NFR.md).
*/
#pragma once
