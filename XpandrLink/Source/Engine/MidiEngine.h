...
public:
    ...
    // (SysEx page/param/patch/mod-edit decode, program nav, mapped CC when no synth input is
    // designated yet). A real, named juce::MidiInput is needed only to exercise the synth-
    // input-auto-detect and MIDI-thru-forward branches, which are out of scope for this seam
    // (see MidiEngineCharacterizationTest.h for what is/isn't covered).
