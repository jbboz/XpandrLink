...
> **Before you connect: back up program slot 99.** XpandrLink uses hardware memory slot 99
> as a dedicated scratchpad for every patch load, library audition, randomizer roll, and
> morph preview. If you have a patch you care about stored in slot 99, save or move it to
> another slot (0–98) before using XpandrLink — the first patch load will overwrite it.
> This is different from the original Windows XplorerEditor, which loads patches directly
> into their own program slot rather than a fixed scratchpad; see
> [Avoiding Accidental Overwrites](#avoiding-accidental-overwrites) below for why XpandrLink
> works this way.
...
This is a deliberate departure from the original Windows XplorerEditor, which sends a program change to the patch's own embedded slot number and loads it there directly — no scratchpad. XpandrLink instead redirects every load to slot 99 so that browsing, auditioning, and editing can never silently overwrite one of your other 98 saved patches.
